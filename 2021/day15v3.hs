...
initSt graph = St {dists = Map.singleton (source graph) 0,
                   ...
                   pq = IntMap.singleton 0 [source graph]}
  ...
  where
    checkNeighbor st v
      ...
      | otherwise = st {dists = Map.insert v newDist (dists st),
                        pq    = IntMap.insertWith (++) newDist [v] (pq st)}
      where
...
takeMin pq
  = do
    (dist_u, u:us) <- IntMap.lookupMin pq
    let pq' = if null us
                then IntMap.deleteMin pq
                else IntMap.updateMin (Just . const us) pq
    return ((dist_u, u), pq')
