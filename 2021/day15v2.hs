...
type PQueue    = Set (Distance,Vertex)
...
initSt graph = St {dists = Map.singleton (source graph) 0,
                   ...
                   pq = Set.singleton (0, source graph)}
...
checkNeighbors graph u dist_u st
  ...
  where
    checkNeighbor st v
      | oldDistIsStillBetter = st
      | otherwise = st {dists = Map.insert v newDist (dists st),
                        pq    = Set.insert (newDist, v) (pq st)}
      where
        newDist = dist_u + graph ! v
        oldDistIsStillBetter
